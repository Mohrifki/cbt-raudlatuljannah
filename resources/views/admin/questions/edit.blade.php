        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <form action="<?= route('admin.questions.update', $question) ?>" method="POST" class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <select name="subject_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                            ...
                            @foreach ($subjects as $subject)
                                <option value="<?= $subject->id ?>" <?= old('subject_id', $question->subject_id) == $subject->id ? 'selected' : '' ?>><?= e($subject->name) ?></option>
                            @endforeach
                        </select>
                ...
                <div class="w-full sm:w-40">
                    ...
                    <input type="number" name="score" value="<?= old('score', $question->score) ?>" min="1" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 bg-green-600 text-white font-semibold px-6 py-2.5 rounded-lg hover:bg-green-700 shadow"><i class="fa-solid fa-floppy-disk"></i> Update Soal</button>
                </div>
            </form>
        </div>
    </div>
