<x-admin-layout title="Edit Soal">
    <?php $type = old('type', $question->type); ?>
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-gradient-to-r from-green-600 to-emerald-500 px-6 py-5 flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-white font-bold text-lg">Edit Soal</h2>
                    <p class="text-green-50 text-sm">Perbarui isi soal</p>
                </div>
            </div>

            <form action="<?= route('admin.questions.update', $question) ?>" method="POST" class="p-6 space-y-5">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <?php $g = old('grade', $question->grade); ?>
                        <select name="grade"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                            <option value="">Semua Tingkat</option>
                            <option value="10" <?= $g === '10' ? 'selected' : '' ?>>Kelas 10</option>
                            <option value="11" <?= $g === '11' ? 'selected' : '' ?>>Kelas 11</option>
                            <option value="12" <?= $g === '12' ? 'selected' : '' ?>>Kelas 12</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="type" id="type-select"
                            ...
                            <option value="pilihan_ganda" <?= $type === 'pilihan_ganda' ? 'selected' : '' ?>>Pilihan
                                Ganda</option>
                            <option value="coding" <?= $type === 'coding' ? 'selected' : '' ?>>Coding (Informatika)
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="hidden" name="question" id="question-input"
                        value="<?= e(old('question', $question->question)) ?>">
                ...
                <div id="pg-options" class="space-y-3"
                    style="display: <?= $type === 'pilihan_ganda' ? 'block' : 'none' ?>;">
                    ...
                    @foreach (['a', 'b', 'c', 'd', 'e'] as $opt)
                        <?php $optVal = data_get($question, 'option_' . $opt); ?>
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <input type="text" name="option_<?= $opt ?>"
                                value="<?= e(old('option_' . $opt, $optVal)) ?>"
                ...
                <div id="essay-answer" style="display: <?= $type === 'essay' ? 'block' : 'none' ?>;">
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Kunci Jawaban / Rubrik <span
                            class="text-gray-400 font-normal">(opsional)</span></label>
                    <input type="hidden" name="answer_key" id="answer-input"
                        value="<?= e(old('answer_key', $question->answer_key)) ?>">
                    <div id="answer-editor"><?= old('answer_key', $question->answer_key) ?></div>
                </div>

                <?php $lang = old('language', $question->language); ?>
                <div id="coding-fields" style="display: <?= $type === 'coding' ? 'block' : 'none' ?>;"
                    class="space-y-3">
                    <div class="w-full sm:w-60">
                        ...
                        <select name="language" id="language-select"
                            ...
                            <option value="javascript" <?= $lang === 'javascript' ? 'selected' : '' ?>>JavaScript
                            </option>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Awal / Starter Code <span
                                class="text-gray-400 font-normal">(opsional)</span></label>
                ...
                <div class="w-full sm:w-40">
                    ...
                    <input type="number" name="score" value="<?= old('score', $question->score) ?>"
                        min="1"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                    @error('score')
                        <p class="text-red-600 text-sm mt-1"><?= $message ?></p>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit"
                        class="inline-flex items-center gap-2 bg-green-600 text-white font-semibold px-6 py-2.5 rounded-lg hover:bg-green-700 shadow"><i
                            class="fa-solid fa-floppy-disk"></i> Update Soal</button>
                </div>
            </form>
        </div>
    </div>
