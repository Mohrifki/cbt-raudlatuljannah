...
<head>
    ...
    <style>
        ...
        @media (min-width: 768px) {
            ...
            #nav-toggle:checked~.app-content {
                padding-left: 0;
            }
        }
    </style>
    ...
    <aside class="app-sidebar fixed inset-y-0 left-0 z-40 w-64 bg-white border-r flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
            <a href="<?= route('admin.dashboard') ?>"
                class="flex items-center gap-3 px-4 py-2.5 rounded-lg <?= request()->routeIs('admin.dashboard') ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-100' ?>">
                <i class="fa-solid fa-house w-5 text-center"></i> Dashboard
            </a>
            <a href="<?= route('admin.users.index') ?>"
                class="flex items-center gap-3 px-4 py-2.5 rounded-lg <?= request()->routeIs('admin.users.*') ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-100' ?>">
                <i class="fa-solid fa-users w-5 text-center"></i> Manajemen User
            </a>
            <a href="<?= route('admin.subjects.index') ?>"
                class="flex items-center gap-3 px-4 py-2.5 rounded-lg <?= request()->routeIs('admin.subjects.*') ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-100' ?>">
                <i class="fa-solid fa-book w-5 text-center"></i> Mata Pelajaran
            </a>
            <a href="<?= route('admin.classes.index') ?>"
                class="flex items-center gap-3 px-4 py-2.5 rounded-lg <?= request()->routeIs('admin.classes.*') ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-100' ?>">
                <i class="fa-solid fa-school w-5 text-center"></i> Kelas
            </a>
            <a href="<?= route('admin.questions.index') ?>"
                class="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-white/10 transition">
                <i class="fa-solid fa-file-circle-question w-5 text-center"></i>
                <span>Bank Soal</span>
            </a>
        </nav>
        <div class="p-4 border-t shrink-0">
            <form method="POST" action="<?= route('logout') ?>">
                @csrf
                <button type="submit"
                    class="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-red-600 hover:bg-red-50">
    ...
    <div class="app-content">
        <header class="h-16 bg-white border-b flex items-center justify-between px-4 sm:px-6 sticky top-0 z-20">
            <div class="flex items-center gap-3">
                <label for="nav-toggle" class="text-gray-600 text-xl cursor-pointer hover:text-green-600"
