        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <form action="<?= route('admin.questions.store') ?>" method="POST" class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <select name="subject_id"
                            ...
                            @foreach ($subjects as $subject)
                                <option value="<?= $subject->id ?>"
                                    <?= old('subject_id') == $subject->id ? 'selected' : '' ?>><?= e($subject->name) ?>
                                </option>
                            @endforeach
                        </select>
                        @error('subject_id')
                    ...
                    <div>
                        ...
                        <select name="type" id="type-select"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                            <option value="essay" <?= old('type', 'essay') === 'essay' ? 'selected' : '' ?>>Essay
                            </option>
                            <option value="pilihan_ganda" <?= old('type') === 'pilihan_ganda' ? 'selected' : '' ?>>
                                Pilihan Ganda</option>
                            <option value="coding" <?= old('type') === 'coding' ? 'selected' : '' ?>>Coding
                                (Informatika)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Rumus: klik tombol formula (√x) lalu tulis LaTeX, mis. <code>E
                            = mc^2</code> atau <code>\frac{a}{b}</code>.</p>
                    @error('question')
                        <p class="text-red-600 text-sm mt-1"><?= $message ?></p>
                    @enderror
                </div>
                ...
                <div class="w-full sm:w-40">
                    ...
                    <input type="number" name="score" value="<?= old('score', 1) ?>" min="1"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                    @error('score')
                        <p class="text-red-600 text-sm mt-1"><?= $message ?></p>
                    @enderror
                ...
                <div id="coding-fields" style="display:none;" class="space-y-3">
                    <div class="w-full sm:w-60">
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Bahasa Pemrograman</label>
                        <select name="language" id="language-select"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                            <option value="python" <?= old('language') === 'python' ? 'selected' : '' ?>>Python
                            </option>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Awal / Starter Code <span
                                class="text-gray-400 font-normal">(opsional)</span></label>
                        <textarea name="starter_code" id="starter-code" rows="10"><?= old('starter_code') ?></textarea>
                        <p class="text-xs text-gray-400 mt-1">Kode awal yang muncul di editor siswa saat ujian coding.
                        </p>
                    </div>
                </div>
            </form>
